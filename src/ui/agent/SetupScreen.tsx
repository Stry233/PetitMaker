...
export function SetupScreen({ top, height, ps, notice, onClearNotice, onStartBuilding }: SetupScreenProps) {
  ...
  const keyHelpRef = useRef<HTMLDivElement>(null);
  const keyHelpFade = useScrollFade(keyHelpRef, 'y');
  ...
  if (notice) {
    ...
    return (
      <div
        style={{
          position: 'absolute', left: px(frame.x), top: px(top), width: px(frame.w), height: px(height),
    ...
    below = (
      <div style={{ display: 'flex', flexDirection: 'column', gap: px(26) }}>
        ...
        <motion.button
          ...
          transition={springs.stiff}
          style={{
            width: px(frame.w), height: px(92), clipPath: squircleClip(px(frame.w), px(92), px(30)),
  ...
  return (
    <div
      style={{
        position: 'absolute', left: px(frame.x - PAD), top: px(top - PAD), width: px(frame.w + PAD * 2), height: px(height + PAD),
