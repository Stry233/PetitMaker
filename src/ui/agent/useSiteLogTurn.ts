...
export function useSiteLogTurn(args: { draft: string; setDraft(v: string): void }): SiteLogTurn {
  const t = useT();
  ...
  const runTurn = async (userText: string, opts: { resumeBpId?: number; slip?: boolean } = {}) => {
    ...
    const deps: AgentToolDeps = {
      ...
      // The painted region, read from the store: the turn's scope is whatever the user has painted
      // when it starts, not what a render happened to hand this hook.
      getRegion: () => region,
    ...
    const runDelegate = async (task: string): Promise<string> => {
      ...
      const sub = await runAgentTurn({
        ...
        mapContext: buildMapContext(gridState, region, deps),
    ...
    try {
      ...
      const history = await runSiteLogTurn({
        ...
        userText,
        mapContext: buildMapContext(gridState, region, deps) + resumeLine,
