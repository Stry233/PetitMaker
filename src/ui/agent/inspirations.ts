 ...
 *
 * `exclude` is what is on screen right now: "another batch" has to be another batch, and drawing
 * from the whole pool leaves a one-in-thirty-two chance per row of handing back the same phrase.
 * Held out of the draw rather than rejected after it, so the guarantee does not depend on luck.
 */
export function sampleInspirations(n = 3, exclude: readonly Inspiration[] = []): Inspiration[] {
  const held = new Set(exclude.map((e) => e.key));
  const pool = INSPIRATIONS.filter((i) => !held.has(i.key));
  let picked = drawDistinct(n, pool);
  for (let attempt = 0; attempt < 4; attempt++) {
    const icons = new Set(picked.map((p) => p.icon));
    if (icons.size === picked.length) break;
    picked = drawDistinct(n, pool);
  }
  return picked;
}
