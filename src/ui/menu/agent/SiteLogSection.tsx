/*
 * SiteLogSection — the AI Agent mode body inside GeneratePanel (spec §UI.1),
 * Composes the four zones below the mode row:
 * header row → dock → the site log → composer, or the setup screen when the
 * gear is open / no key exists / an incident needs attention.
 *
 ...
 * back to design px through the scale) — the one place the PSD grid flexes.
 *
...
/** Vertical rhythm (design px, prototype ×2): header 76 tall, 22 gaps,
 *  composer block at the panel's fixed foot. */
const HEADER_H = 76;
const GAP = 22;
const COMPOSER_TOP = 946;
...
export function SiteLogSection({ top, region, isSelecting, onSelectRegion, onClearRegion }: Props) {
  const t = useT();
  ...
  const turn = useSiteLogTurn({ region, draft, setDraft });
  ...
  const logHeight = top + COMPOSER_TOP - GAP - logTop;
  ...
  /* ── screen transition: the setup <-> chat slide owns ALL movement here
        (agent-mode entry itself is animated by GeneratePanel's clip fade); a
        per-row bottom-up cascade on top of the slide read as double motion ── */
  const slide = {
  ...
  return (
    ...
    <motion.div key="log" custom={slideDir} variants={slide} initial={firstScreen.current ? 'center' : 'enter'} animate="center" exit="exit"
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        <HeaderRow
          top={top}
          regionCells={isSelecting || region.length > 0 ? region.length : 0}
          onRegionToggle={() => (region.length > 0 || isSelecting ? onClearRegion() : onSelectRegion())}
