/**
 * The cursor images: one PNG per CursorId, plus a pre-badged variant for each cursor that can
 * refuse.
 *
 * `src/assets/cursors/` is GENERATED from the project's design source, so a change to a cursor
 * is a change to the artwork it comes from.
 *
