...
import { TerrainToolButton } from './TerrainToolButton';
...
export function SmartBuild({ surface, centre, active }: {
  ...
  const press = useCallback(() => setEditMode({ tool: 'smart', macro: action.id }), [setEditMode, action.id]);
  useEffect(() => pictured ? undefined : offerSmartBuild(press), [press, pictured]);

  return <div {...helpTargetAttr('smart')}>
    <TerrainToolButton glyph={SMART.cellGlyph} label={t('smart.build')}
