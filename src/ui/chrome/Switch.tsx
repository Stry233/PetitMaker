...
/**
 * The one cozy toggle switch — 48×26 track, 20px knob sliding 3px↔25px, tile-yellow
 * when on. Shared by SettingsModal (grid / chunk toggles) and the export controls.
...
export function Switch({ on, onClick, label, disabled }: { on: boolean; onClick: () => void; label?: string; disabled?: boolean }) {
  ...
  return (
    <motion.button
      ...
      animate={{ backgroundColor: on ? colors.tileYellow : colors.trackOff }}
