...
const excludedSections = new Set([
  ...
  'shortcuts-presets', 'undo-agent', 'settings-immersive', 'share-confirm',
]);
const excludedKeys = new Set(['help.stylize.co1', 'help.share.co1', 'help.region.co1', 'help.candidates.own_b2', 'help.settings.about_b1', 'help.sharelook.attribution_b1', 'help.share.textlimit_b1']);
const replace = (key: string) => replacements[key] ?? key;

export const LITE_HELP_SURFACE_IDS = ['frame-overview', 'frame-modes', 'frame-topright', 'frame-rail', 'frame-bar', 'tour', 'layers', 'notes-row', 'notes-entry', 'object-shelf', 'autotrim-modes', 'context-menu', 'delete-confirm', 'generate-shelf', 'generate-shelf-island', 'island-height', 'island-richness'] as const;
...
export function liteHelpContent(page: HelpPage): HelpPage {
  return {
    ...
    entryKey: replace(page.entryKey),
