...
import { ModalShell, useModalExiting } from '../../../primitives/ModalShell';
...
export function HelpModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // While the whole shell is closing, the page-swap crossfade below must not also run its own exit
  // (see ModalShell's `useModalExiting` doc) — the current page rides the card out as one unit.
  ...
  useEffect(() => {
    ...
    if (HELP_PAGES[target]) {
      setPage(target);
      pendingAnchor.current = helpTarget.anchor ?? null;
    }
    setHelpTarget(null);
  }, [open, helpTarget, setHelpTarget]);

  // Landing on a page scrolls to its top, or to the deep link's own section, which flashes once so
  // the eye lands where the link pointed. The flash eases IN as well as out (two stages: rAF commits
  // the transparent starting frame before the highlight is set, then a hold, then the release) so
  // the highlight glides in rather than snapping on.
  useEffect(() => {
  ...
  // Stable across keystrokes: this is the memoized page body's one prop besides the page itself,
  // so a `go` rebuilt per render would re-render every live demo on each character typed.
  ...
  const go = useCallback((id: HelpPageId, anchor?: string) => {
    ...
    setQuery('');
    setHits([]);
    if (anchor && id === pageRef.current) {
      // Same page: the page effect will not re-run, so honour the anchor by hand.
      const host = scrollRef.current;
      const el = host?.querySelector<HTMLElement>(`#help-${anchor}`);
        ...
        <div ref={scrollRef} style={{ overflowY: 'auto', minWidth: 0, overflowAnchor: 'none' }}>
          <AnimatePresence mode="wait" initial={false}>
            <motion.div
              ...
              transition={springs.stiff}
            >
              <PageView page={HELP_PAGES[page]} onGo={go} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
