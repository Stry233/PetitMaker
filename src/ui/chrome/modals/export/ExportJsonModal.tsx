...
export function ExportJsonPanel({ open, onDone }: { open: boolean; onDone: () => void }) {
  ...
  const mapReview = useMapReview(open, false, false);
  const exportDisabled = exporting || mapReview.pending || !textReview.allowed || mapReview.result?.status === 'blocked';
  const noticeRevision = JSON.stringify([notesOn, notesVal, composing, generationOn, provenanceOn, historyOn, historyDepthKey, sessionOn, statsOn, catalogOn, pretty]);
        ...
        <Expand open={notesOn}>
          ...
          <div onCompositionStartCapture={() => setComposing(true)} onCompositionEndCapture={() => setComposing(false)} style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '8px 8px 2px' }}>
            ...
            {issue && <div role="alert" style={rowDesc}>
              {issue === 'unavailable' ? t('export.review.unavailable') : issue === 'too-long' ? t('export.review.too_long') : t(reviewReasonKey(issue.reason), { fields: issue.fields.map(field => t(field === 'title' ? 'export.field_title' : field === 'description' ? 'export.field_desc' : 'exportjson.author')).join(', ') })}
