...
import { font, colors, inkTint, springs, exitTransition, radii, buttonMotion, cursors } from '../../design/styles';
import {
  skin, windowCard, windowLabel, windowMenu, windowMenuItem, windowMenuItemHover, windowPill,
  windowPrimary, windowRow, windowTitle,
...
// Compact modal-row slider: a dark track with an active-yellow fill and knob, a soft knob shadow
// and a value bubble that pops over the knob, laid out for a fixed-width settings row — a
// full-width track under the label, and a reset pill beside it.
//
...
function UiScaleSlider({ label }: { label: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      >
        <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 10, transform: 'translateY(-50%)', background: skin.ink, borderRadius: 999, opacity: 0.9 }} />
        <div style={{ position: 'absolute', top: '50%', left: 0, width: `${ratio * 100}%`, height: 10, transform: 'translateY(-50%)', background: skin.active, borderRadius: 999 }} />
        <div style={{ position: 'absolute', top: '50%', left: `${ratio * 100}%`, width: 20, height: 20, transform: 'translate(-50%, -50%)', borderRadius: '50%', background: skin.active, boxShadow: `0 2px 6px ${inkTint(0.3)}` }} />
        ...
        <AnimatePresence>
          {(hover || dragging) && (
            <motion.div key="bubble"
              ...
              transition={springs.stiff}
              style={{ position: 'absolute', left: `${ratio * 100}%`, bottom: '100%', marginBottom: 12, transformOrigin: 'bottom center', background: skin.ink, color: skin.onDark, fontFamily: font.family, fontWeight: 900, fontSize: 14, lineHeight: 1, padding: '5px 11px', borderRadius: 10, whiteSpace: 'nowrap', pointerEvents: 'none', boxShadow: `0 4px 12px ${inkTint(0.32)}` }}>
              {`${pct}%`}
...
// make label + control together wider than the fixed 380px card. The shared row's `flexWrap` lets
// the control drop to its own line below the label instead of overflowing the card edge or wrapping
// mid-word inside a pill.
const rowStyle: CSSProperties = { ...windowRow };

const labelStyle: CSSProperties = { ...windowLabel };
...
const dropdownItemStyle = (active: boolean): CSSProperties => windowMenuItem(active);

// Pill chip on the About row's right side — surfaces the build number and opens the About page on
// click. The same pill every in-row action in this window wears.
const aboutChipStyle: CSSProperties = { ...windowPill() };

// Reset-to-100% pill on the UI-scale row — the same pill, sized down for a secondary in-row action.
const resetChipStyle: CSSProperties = { ...windowPill(), fontSize: 12, padding: '5px 11px' };

const dangerChipStyle: CSSProperties = { ...windowPill('danger') };
...
export function SettingsModal({
  ...
  // sits inside `ModalShell`'s chrome-scale CSS `zoom` (see ui/design/scale.tsx),
                ...
                >
                  {LOCALES.map(({ code, label }) => (
                    <motion.button
                      ...
                      whileHover={{ backgroundColor: windowMenuItemHover(locale === code) }}
                      whileTap={{ scale: 0.97 }}
        ...
        <AnimatePresence>
          {confirmReset && (
            <motion.div
              ...
              // CARD_ROW_GAP, jumping every row below it. Ending the exit at
