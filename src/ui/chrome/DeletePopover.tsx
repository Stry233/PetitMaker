...
import { peelTerrainAction } from './object-actions';
import { deleteGroup, reportDeleteGroup } from './group-actions';
import { useT, localizedName } from '../../i18n/context';
import { colors, radii, shadows, font, btnBase, springs, pressable, exitTransition, z } from '../styles';
    ...
    if (sel.kind === 'object') {
      ...
      if (obj) {
        ...
        confirm = () => {
          ...
          // refused delete. removeObjectAction itself gates and plays the collapse animation, so
          // this component no longer needs its own pre-validation probe.
          const result = deleteGroup(executor, useEditorStore.getState().gridState!, [obj.id]);
      ...
      if (cell?.terrain) {
        ...
        confirm = () => {
          peelTerrainAction(executor, sel.x, sel.y, cell);
          useEditorStore.getState().clearSelection();
          dismiss();
        };
      }
