...
import { colors, font, radii, cursors } from '../../styles';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: colors.textSecondary, lineHeight: 1.4 }}>{t(`export.preset_${options.preset}_desc`)}</div>
      </div>
          ...
          <Expand open={codeDropped}>
            <div style={{ paddingTop: 8 }}>
              <div style={codeWarn}>
                ...
                <span>{t('export.code_size_warn')}</span>
              </div>
            </div>
          </Expand>
        </div>
...
const inputStyle: CSSProperties = { fontFamily: font.family, fontSize: 13.5, fontWeight: 600, color: colors.frameDark, background: colors.surfacePrimary, border: `1.5px solid ${colors.inkBorder}`, borderRadius: radii.md, padding: '9px 11px', resize: 'none', outline: 'none', width: '100%', boxSizing: 'border-box' };
const capStyle: CSSProperties = { fontSize: 12.5, fontWeight: 800, color: colors.textSecondary };
...
const codeWarn: CSSProperties = {
  ...
  color: colors.frameDark, background: 'rgba(255,179,71,0.14)', border: '1px solid rgba(255,179,71,0.42)',
  borderRadius: radii.sm, padding: '7px 9px',
};
const detailsBtn: CSSProperties = { alignSelf: 'flex-start', background: 'transparent', border: 'none', cursor: cursors.clickable, fontFamily: font.family, fontSize: 12.5, fontWeight: 800, color: colors.textSecondary, padding: 0 };
const rowLabel: CSSProperties = { fontSize: 14, fontWeight: 700, color: colors.frameDark };

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}><span style={{ fontSize: 12.5, fontWeight: 700, color: colors.frameDark }}>{label}</span>{children}</label>;
}
...
function Label({ text, help }: { text: string; help?: string }) {
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 14, fontWeight: 700, color: colors.frameDark }}>{text}{help && <HelpBubble text={help} />}</span>;
}
