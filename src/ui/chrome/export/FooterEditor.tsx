...
export function FooterEditor({ value, onChange, samples, t }: {
  ...
  const rowStyle = (idx: number): CSSProperties => ({ ...menuRow, background: idx === menuIndex ? inkTint(0.09) : 'transparent' });

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <style>{`
        .ppfe-field .ppfe-chip{display:inline-flex;align-items:center;line-height:1;background:${colors.tileYellow};color:${colors.frameDark};border-radius:999px;padding:5px 9px;margin:0 1px;font-weight:800;font-size:11.5px;white-space:nowrap;user-select:none;}
        .ppfe-field .ppfe-fill{background:#cfe3d6;}
        .ppfe-field:empty:before{content:attr(data-ph);color:${colors.textSecondary};opacity:.7;}
      `}</style>
          ...
          return (
            <>
              ...
              <motion.div
                ...
                style={{ ...menuStyle, left: menu.left / chrome, maxHeight: maxH / chrome, ...pos }}
              >
...
const fieldStyle: CSSProperties = { flex: 1, minHeight: FIELD_BOX, background: colors.surfacePrimary, border: `1.5px solid ${colors.inkBorder}`, borderRadius: radii.md, padding: `${FIELD_PAD_Y}px 10px`, fontFamily: font.family, fontSize: 13, fontWeight: 600, color: colors.frameDark, lineHeight: `${FIELD_LINE}px`, outline: 'none', boxSizing: 'border-box', overflowWrap: 'anywhere' };
// Exactly one line tall — same box height as the field's collapsed state, so the button and the
// input align at the top.
const addBtn: CSSProperties = { flex: 'none', height: FIELD_BOX, display: 'inline-flex', alignItems: 'center', boxSizing: 'border-box', border: `1.5px dashed ${inkTint(0.25)}`, background: 'transparent', cursor: cursors.clickable, borderRadius: radii.md, padding: '0 12px', fontFamily: font.family, fontWeight: 800, fontSize: 11.5, color: colors.textSecondary, whiteSpace: 'nowrap' };
const hintStyle: CSSProperties = { fontSize: 11, fontWeight: 600, color: colors.textSecondary, opacity: 0.85 };
const menuStyle: CSSProperties = { position: 'fixed', zIndex: 301, width: MENU_W, overflowY: 'auto', background: '#fffdf5', border: `1.5px solid ${colors.inkBorder}`, borderRadius: radii.md, boxShadow: `0 10px 28px ${inkTint(0.22)}`, padding: 5, display: 'flex', flexDirection: 'column', gap: 2 };
const menuRow: CSSProperties = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, border: 'none', cursor: cursors.clickable, borderRadius: radii.sm, padding: '7px 9px', fontFamily: font.family, fontSize: 12.5, color: colors.frameDark, textAlign: 'left', width: '100%' };
const menuSample: CSSProperties = { fontSize: 11, fontWeight: 600, color: colors.textSecondary, fontVariantNumeric: 'tabular-nums', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
const menuSep: CSSProperties = { height: 1, background: inkTint(0.1), margin: '3px 4px' };
