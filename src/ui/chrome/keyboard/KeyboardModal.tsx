 ...
 * holder), Clear, or Reset. Everything reads from the command REGISTRY (ui/keybindings/commands) +
 * the user OVERRIDE store (ui/keybindings/store) — adding a command anywhere makes it appear +
...
import { colors, font, radii, inkTint, springs, shadows, buttonMotion, modalTitle, cursors } from '../../styles';
import { ModalShell } from '../ModalShell';
import { showToast } from '../Toast';
import { downloadBlob } from '../../../io/image-export';
import { COMMANDS, COMMAND_BY_ID, type EditorCommand } from '../../keybindings/commands';
import { aliasIndex, bindingIndex, effectiveCombo, useKeybinds } from '../../keybindings/store';
import { PRESETS, detectPreset, serializeKeybinds, parseKeybinds } from '../../keybindings/presets';
import {
  KEY_ROWS, NUMPAD, NUMPAD_COLS, NUMPAD_ROWS_N, NAV, NAV_COLS, NAV_ROWS_N,
  CATEGORY_COLOR, CATEGORY_ORDER, comboFor, comboFromEvent, prettyCombo, type KeyDef, type Layer,
} from './layout';
...
const cardStyle: CSSProperties = { overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: 24 };
...
// until the v-bar is scrolled down. overflow:auto hides each cozy bar until needed; safe-center
// centers the board when it fits and left-aligns it when it scrolls; scrollbar-gutter reserves the
// vertical track on both sides (symmetric, no shift); flexShrink lets it absorb a height squeeze.
...
const titleStyle: CSSProperties = { ...modalTitle, marginBottom: 16, flexShrink: 0 };

// Filled cozy pills matching the other modals (Settings/About): no border, surfaceSecondary fill,
// accent for the primary action, dangerBg for destructive. Paired with `buttonMotion` on the element.
const pillBase: CSSProperties = {
  fontFamily: font.family, fontSize: 14, fontWeight: 800, padding: '8px 16px',
  borderRadius: radii.pill, border: 'none', cursor: cursors.clickable, whiteSpace: 'nowrap',
};
function pill(variant: 'quiet' | 'primary' | 'danger', disabled = false): CSSProperties {
  const base: CSSProperties = { ...pillBase, cursor: disabled ? cursors.blocked : cursors.clickable, opacity: disabled ? 0.4 : 1 };
  if (variant === 'primary') return { ...base, background: colors.accentPrimary, color: colors.frameDark };
  if (variant === 'danger') return { ...base, background: colors.dangerBg, color: colors.dangerText };
  return { ...base, background: colors.surfaceSecondary, color: colors.frameDark };
}

// Layer chips = a filled segmented control: active is the dark ink pill, inactive the quiet fill.
function chip(active: boolean): CSSProperties {
  return {
    ...pillBase, fontWeight: active ? 800 : 700, padding: '7px 16px',
    background: active ? colors.frameDark : colors.surfaceSecondary,
    color: active ? colors.panelCream : colors.textSecondary,
  };
}

const comboChip: CSSProperties = {
  fontFamily: font.family, fontSize: 13.5, fontWeight: 800, color: colors.panelCream,
  background: colors.frameDark, padding: '5px 12px', borderRadius: 8, whiteSpace: 'nowrap',
};

// Shared cozy dropdown menu (mirrors the Settings language menu): surfacePrimary card, soft shadow,
// tileYellow active row; items animate their background on hover instead of flashing from transparent.
const menuStyle: CSSProperties = {
  position: 'absolute', top: 'calc(100% + 6px)', left: 0, minWidth: 172, zIndex: 6,
  background: colors.surfacePrimary, borderRadius: radii.lg, boxShadow: shadows.s2,
  padding: 6, display: 'flex', flexDirection: 'column', gap: 2, transformOrigin: 'top left',
};
function menuItem(active: boolean): CSSProperties {
  return {
    display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, width: '100%',
    fontFamily: font.family, fontSize: 14, fontWeight: active ? 800 : 700, border: 'none', textAlign: 'left',
    cursor: cursors.clickable, padding: '8px 14px', borderRadius: radii.md, whiteSpace: 'nowrap',
 ...
 * Restraint-first per ANIMATION_PLAN: one short cascade (~0.3s total), spring physics, and no
 * movement at all under reduced motion.
 */
...
export function KeyboardModal({ open = true, onClose }: KeyboardModalProps) {
  ...
  const renderKey = (key: KeyDef, ri: number, ki: number) => {
    ...
    const st: CSSProperties = {
      ...
      boxShadow: isSelected ? `0 0 0 3px ${colors.accentPrimary}` : undefined,
    ...
    const bg = modActive
      ...
      : key.fixed ? inkTint(0.07)
        : cmd ? CATEGORY_COLOR[cmd.category] : colors.panelCream;
          ...
          <AnimatePresence initial={false}>
            {cmd && (
              ...
              >
                ...
                <span style={{
                  fontSize: 11.5, fontWeight: 700, lineHeight: 1.12, color: colors.frameDark, textAlign: 'center',
  ...
  const detail = (() => {
    if (!selected) return <span style={{ fontSize: 14, color: colors.textSecondary, fontFamily: font.family }}>{t('kbd.hint')}</span>;
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 800, color: colors.frameDark, fontFamily: font.family }}>{t(selected.labelKey)}</span>
        <span style={recording ? { ...comboChip, background: colors.accentPrimary, color: colors.frameDark } : comboChip}>
        ...
        {!reserved && (() => {
          ...
          return (
            <>
              ...
              <motion.button {...(clearOff ? {} : buttonMotion)} style={pill('quiet', clearOff)} disabled={clearOff} onClick={() => { clear(selected.id); setNote(null); }}>{t('kbd.clear')}</motion.button>
              <motion.button
                {...(resetOff ? {} : buttonMotion)}
                style={pill('quiet', resetOff)} disabled={resetOff}
        ...
        {note && <span style={{ fontSize: 13, color: colors.textSecondary, fontFamily: font.family }}>{note}</span>}
      </div>
    );
  })();
  ...
  return (
    <ModalShell open={open} onClose={onClose} width={width} maxVwPct={96} maxVh={92} cardStyle={cardStyle} ariaLabel={t('modal.help_title')}>
        ...
        <div style={{ position: 'relative', flex: '1 1 auto' }}>
          <input
            ...
            style={{
              ...
              padding: '10px 16px', borderRadius: radii.pill, border: `1.5px solid ${colors.inkBorder}`,
              background: colors.surfacePrimary, color: colors.frameDark, outline: 'none',
            }}
          />
          <AnimatePresence>
            {searchHits.length > 0 && (
                ...
                {searchHits.map((c) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileHover={{ backgroundColor: colors.surfaceSecondary }}
                      whileTap={{ scale: 0.97 }}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', border: 'none', background: colors.surfacePrimary, borderRadius: radii.md, padding: '9px 14px', cursor: cursors.clickable }}
                    >
                      <span style={{ width: 11, height: 11, borderRadius: 3, background: CATEGORY_COLOR[c.category], flex: '0 0 auto' }} />
                      <span style={{ flex: '1 1 auto', textAlign: 'left', fontSize: 14, fontWeight: 700, color: colors.frameDark, fontFamily: font.family }}>{t(c.labelKey)}</span>
                      <span style={{ fontSize: 12.5, fontWeight: 700, color: colors.textSecondary, fontFamily: font.family }}>{combo ? prettyCombo(combo) : t('kbd.unbound')}</span>
                    </motion.button>
                  );
                })}
              </motion.div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexShrink: 0, alignItems: 'center' }}>
        ...
        <div ref={presetRef} style={{ position: 'relative' }}>
          <motion.button {...buttonMotion} onClick={() => setPresetOpen((o) => !o)} style={{ ...pill('quiet'), display: 'inline-flex', alignItems: 'center', gap: 7 }}>
            <span style={{ color: colors.textSecondary, fontWeight: 700 }}>{t('kbd.preset')}</span>
            ...
            <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden style={{ transition: 'transform 0.18s ease', transform: presetOpen ? 'rotate(180deg)' : 'none' }}>
              <path d="M2 3.5 L5 6.5 L8 3.5" fill="none" stroke={colors.frameDark} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </motion.button>
          <AnimatePresence>
            {presetOpen && (
              ...
              >
                {PRESETS.map((p) => (
                  <motion.button
                    ...
                    whileHover={{ backgroundColor: activePreset === p.id ? colors.tileYellow : colors.surfaceSecondary }}
                    whileTap={{ scale: 0.97 }}
      ...
      {/* ONLY the keyboard scrolls (header above + footer below stay pinned). One region scrolls both
          ...
          height pressure (high UI zoom) and scrolls; both cozy bars anchor to the scrollport edges. */}
      <div style={{ ...kbdRegion, height: keyboardH + 20 }}>
      ...
      <div style={{ marginTop: 16, flexShrink: 0, minHeight: 44, display: 'flex', alignItems: 'center', padding: '11px 16px', borderRadius: radii.md, background: inkTint(0.05) }}>
      ...
      <div style={{ marginTop: 14, flexShrink: 0, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        {CATEGORY_ORDER.map((cat) => (
          <span key={cat} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 600, color: colors.textSecondary, fontFamily: font.family }}>
