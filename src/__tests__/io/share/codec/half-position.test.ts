// Half-cell positions use dedicated model variants. Whole-cell maps keep the compact variants,
// while half-cell maps must round-trip exact coordinates through both the codec and image path.
...
const unhex = (value: string) => Uint8Array.from(value.match(/../g) ?? [], (byte) => Number.parseInt(byte, 16));
...
/** Fixed whole-cell fixture independent of generator output. */
...
describe('share codec: half positions are additive', () => {
  it('decodes the pinned pre-mask frame 4 bytes', async () => {
    const bytes = unhex('');
    const decoded = await decodeMapPayload(bytes);
    expect(canonicalBytes(decoded.canonical)).toEqual(canonicalBytes(canonicalize(wholeMap())));
  });
  ...
  it('names a half-capable shape only when the map holds one', async () => {
    // The frame's fourth byte selects the model variant.
  ...
  it('refuses a position a shape cannot carry', () => {
    ...
    // Whole-cell variants cannot store offsets; no variant supports quarter-cell positions.
  ...
  it('costs nothing measurable per object to carry the half-capable shape', () => {
    // One adaptive symbol per object keeps the half-position flag below a quarter bit each here.
    const template = getMapTemplate('hexia');
    const cells = tokensOf(canonicalize(createBlankGridState('hexia')).cells).map(parseToken);
  ...
  it('carries a half anchor the whole way out and back', async () => {
    // Exercise image rendering, raster decoding, save loading, and import validation together.
    const state = wholeMap();
    place(state, 'e', 'ramp-teak-stair', 40.5, 30, 0, 1);
    place(state, 'f', 'bridge-teak', 44, 50.5, 90, 0, 4);
