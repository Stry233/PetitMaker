// useTour's advance/skip/finish state machine is step-content-agnostic; the shell's own step list
// is what exercises it here.
...
describe('useTour hook', () => {
  ...
  it('reaching the end opens the send-off', () => {
    const { result } = renderHook(() => useTour(TOUR_STEPS));
    act(() => { startTour(); });
