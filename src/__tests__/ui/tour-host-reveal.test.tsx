 ...
 *
 * `App.handleTourStep` carries a comment that its menu change must be a direct, synchronous state
 * set inside the overlay's layout-phase callback, because the step's target (the phone card's
 * tiles) does not exist in the DOM until that set lands. Every other tour test stubs
...
/**
 * The app's phone card, reduced to the one fact the tour depends on: the collapsed phone and the
 * card's tiles are never in the DOM at the same time, and only `onStepEnter` decides which.
 */
function Host({ apply }: { apply: (set: (up: boolean) => void, step: TourStep) => void }) {
  const [cardUp, setCardUp] = useState(false);
  return (
    <>
      {cardUp ? <div {...tourTargetAttr('menu-tiles')} /> : <div {...tourTargetAttr('menu-collapsed')} />}
      <TourOverlay onStepEnter={(step) => apply(setCardUp, step)} />
    </>
  );
}

/** Exactly what App.handleTourStep does. */
function applyDirect(setCardUp: (up: boolean) => void, step: TourStep) {
  if (step.menu === 'expand') setCardUp(true);
  if (step.menu === 'collapse') setCardUp(false);
}
...
describe('the host reveals a step\'s target from inside onStepEnter', () => {
  ...
  it('shows the step whose target only comes into being when the step is announced', async () => {
    ...
    clickNext(); // camera -> open
    await waitFor(() => expect(shownStep()).toBe(3)); // the collapsed phone, measured

    clickNext();
    // `menu` is the step whose target the host creates: without the reveal landing before the
    // measurement it is passed over, and the counter jumps straight past 4.
    await waitFor(() => expect(shownStep()).toBe(4));
    expect(screen.getByText('Everything starts here')).toBeTruthy();
    expect(document.querySelector('[data-tour-target="menu-tiles"]')).not.toBeNull();
  });
  ...
  it('passes that step over when the host has not revealed the target by the time it is measured', async () => {
    ...
    clickNext();
    // ONE press, and the tour is over: `menu` and every step after it names a target that is on
    // the card the host never opened, so each is passed over in turn. No step past 3 was ever put
    // on the card, which is what a press would otherwise have been needed to leave.
    await waitFor(() => expect(useEditorStore.getState().tourRunning).toBe(false));
    expect(screen.queryByText('Everything starts here')).toBeNull();
  });
});
