...
describe('measureTarget', () => {
  it('returns null when the target is not on screen', () => {
    expect(measureTarget('menu-tiles')).toBeNull();
  });

  it('measures the marked element', () => {
    render(<div {...tourTargetAttr('menu-tiles')} />);
    const el = document.querySelector('[data-tour-target="menu-tiles"]') as HTMLElement;
    ...
    const rect = measureTarget('menu-tiles');
    expect(rect?.left).toBe(10);
    expect(rect?.width).toBe(30);
  });

  it('ignores an element with no size, which is one that has not laid out yet', () => {
    render(<div {...tourTargetAttr('menu-home')} />);
    expect(measureTarget('menu-home')).toBeNull();
  });
});
