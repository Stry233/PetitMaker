...
describe('exec/runner', () => {
  ...
  it('files a refused order as a finished job with a local reply and no model turn', async () => {
    screening.refuse.add('draw the forbidden thing');
    const turns: ScriptedTurn[] = [textTurn('should never stream')];
    const adapter = createScriptedAdapter(turns);
    const runner = createRunner(makeCfg({ adapterForTest: adapter, uiLocale: 'zh' }));
    runner.send('draw the forbidden thing');
    await flush();
    screening.refuse.clear();
    const events = eventsOf(useAgentSession.getState().log);
    expect(events.map((e) => e.kind)).toEqual(['order', 'assistant', 'jobEnd']);
    const reply = events[1] as Extract<typeof events[number], { kind: 'assistant' }>;
    expect(reply.parts).toEqual([{ kind: 'text', text: '这个请求我无法帮忙，我们继续做地图吧。', done: true }]);
    expect((events[2] as Extract<typeof events[number], { kind: 'jobEnd' }>).outcome).toBe('done');
    expect(runner.active()).toBe(false);
