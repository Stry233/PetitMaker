...
describe('computeLockedCorners — terrain', () => {
  ...
  it('mountain+water diagonal pinch: BOTH free — no special case for mountain+water (a diagonal never pins)', () => {
    ...
    expect(computeLockedCorners(state, roadLookup(state), 5, 5, 'terrain')[3], 'mountain BR rounds toward the diagonal water').toBe(false);
    expect(computeLockedCorners(state, roadLookup(state), 6, 6, 'terrain')[0], 'water TL rounds toward the diagonal mountain').toBe(false);
  });
  ...
  it('a mountain ISLAND (water on all sides) IS fully cuttable — every corner has water on BOTH edges', () => {
    ...
    expect(computeLockedCorners(state, roadLookup(state), 5, 5, 'terrain')).toEqual([false, false, false, false]);
  });
  ...
  it('a mountain at a DIAGONAL water pinch (mountain CONTINUES on the diagonal) is a bank, not a tip — locked', () => {
    ...
    expect(computeLockedCorners(s, roadLookup(s), 6, 5, 'terrain')[2], 'pinch around continuous terrain → locked').toBe(true);
    setTerrain(s, 5, 6, Wt, 0);                             // remove the continuation → now fully in water
    expect(computeLockedCorners(s, roadLookup(s), 6, 5, 'terrain')[2], 'a genuine peninsula tip → free').toBe(false);
  });
  ...
  it("a mountain banking water beside a ground pocket keeps its bank (the user's water/mountain junction)", () => {
    ...
    expect(computeLockedCorners(s, roadLookup(s), 3, 2, 'terrain')[2], 'm*(3,2) BL (W=water, S=pocket) bank → locked').toBe(true);
    expect(computeLockedCorners(s, roadLookup(s), 2, 3, 'terrain')[1], 'm*(2,3) TR (N=water, E=pocket) bank → locked').toBe(true);
  });
...
describe('computeLockedCorners — road', () => {
  ...
  it('isolated road: all 4 free', () => {
    const state = makeState(10, 10);
    addRoad(state, 5, 5);
    const locked = computeLockedCorners(state, roadLookup(state), 5, 5, 'road');
    expect(locked).toEqual([false, false, false, false]);
  });
