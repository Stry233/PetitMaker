...
import { ItemCategory, ObjectCategory, CellZone } from '../../core/model/types';
...
describe('preview3d/object-meshes', () => {
  it('groups objects under their catalog category archetype', () => {
    registerCatalogItem({ id: 'oak', category: ItemCategory.Tree, name: { en: 'Oak' }, emoji: '', width: 1, height: 1, loadValue: 0, rotatable: false, placementMode: 'point', traits: [] });
    const obj: PlacedObject = { id: 'a', catalogId: 'oak', position: { x: 2, y: 2 }, rotation: 0, category: ObjectCategory.Tree, elevation: 0 };
  ...
  it('an item with a bespoke 3D model groups under m:<catalogId> at scale 1', () => {
    ...
    const obj: PlacedObject = { id: 's', catalogId: 'building-stall', position: { x: 3, y: 3 }, rotation: 0, category: ObjectCategory.House, elevation: 0 };
    const groups = buildObjectInstances(emptyGrid(8, 8, [obj]));
  ...
  it('off-catalog self-described objects (plaza) use the platform archetype', () => {
    const plaza: PlacedObject = { id: '__plaza__', catalogId: '__plaza__', position: { x: 1, y: 1 }, rotation: 0, category: ObjectCategory.Facility, elevation: 1, width: 3, height: 2, color: '#e2e8f0', locked: true };
  ...
  it('a 90°-rotated 2×1 object orients via rotationY but keeps unrotated scale', () => {
    registerCatalogItem({ id: 'hall', category: ItemCategory.Building, name: { en: 'Hall' }, emoji: '', width: 2, height: 1, loadValue: 0, rotatable: true, placementMode: 'point', traits: [] });
    const obj: PlacedObject = { id: 'b', catalogId: 'hall', position: { x: 0, y: 0 }, rotation: 90, category: ObjectCategory.House, elevation: 0 };
  ...
  it('a ramp sits at the LOW elevation (n−drop) and spans up one layer, facing the right way', () => {
    registerCatalogItem({ id: 'ramp1', category: ItemCategory.Ramp, name: { en: 'Ramp' }, emoji: '', width: 1, height: 2, loadValue: 0, rotatable: true, placementMode: 'point', traits: [{ type: 'heightDrop', layers: 1 }] });
    const obj: PlacedObject = { id: 'rp', catalogId: 'ramp1', position: { x: 0, y: 0 }, rotation: 0, category: ObjectCategory.House, elevation: 2 };
...
describe('ground-level placement sits on the visible surface', () => {
  it('an elevation-0 object stands on the ground slab, not buried inside it', () => {
    ...
    const flora: PlacedObject = {
      ...
      rotation: 0, category: ObjectCategory.Flora, elevation: 0,
    };
  ...
  it('an elevated object still sits flush on its terrain top', () => {
    const flora: PlacedObject = {
      id: 'f', catalogId: 'flower-bellflower', position: { x: 3, y: 3 },
      rotation: 0, category: ObjectCategory.Flora, elevation: 2,
    };
    const groups = buildObjectInstances(emptyGrid(10, 10, [flora]));
    const inst = [...groups.values()].flat()[0]!;
