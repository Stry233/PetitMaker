...
describe('Maze gates', () => {
  ...
  /**
   * AN INTERIOR END IS A DESTINATION, not a badly-aimed gate. The old model could not express one:
   * it snapped the coordinate to the border ring, so aiming inside the maze silently moved the end
   * to the map's edge. Now the drop decides, and a place inside is a place to REACH — which costs
   * no terrain at all, since a corridor is already walkable.
   */
