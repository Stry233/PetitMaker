...
} from '../../../../tools/generation/placement/route';
...
describe('the alignment discount', () => {
  it('planRoute takes a genuinely longer detour when the discount makes it genuinely cheaper, and stays direct otherwise', () => {
    // Open ground; a "standing street" alignment one row below the direct line. Before straighten()
    // was made cost-aware, this collapsed straight back to the direct line — passable within the
    // geometric slack is all the old rule asked, and a detour that pays for itself only under a
    // profile's own cost function looks, geometrically, like pure waste. This exercises the real
    // pipeline end to end (planRoute → buildLegCells → straighten), not just astar's raw output.
    const W = 30, H = 20;
