...
import type { RouteWorld } from '../../../../tools/generation/placement/route';
import { routeOffers, sameRoute, OFFER_SAME } from '../../../../tools/generation/placement/route-offers';
...
describe('the straight profile is itself at every style', () => {
  it('differs from short even where the map style carries no turn penalty', () => {
    // The reviewer's collapse: astar's turnPenalty>0 gate closed before the profile's own turn
    // cost was consulted, so 'straight' equaled 'short' on every organic-default map. Same comb
    // fixture as above, but at a ZERO-turn-penalty style.
    const W = 60, H = 40;
    const state = makeState(W, H);
    const teeth = [
      { x: 12, gapLo: 18, gapHi: 21 },
