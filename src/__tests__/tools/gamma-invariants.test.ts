 ...
 *
 *   I1  A fillet RESTS: its tier is one above the cell's own support. Higher and it hangs in the
 *       air — a floating wedge in 3D, a wrongly-coloured corner in 2D.
...
function violations(state: GridState): Violation[] {
  ...
  state.cells.forEach((row, y) => row.forEach((cell, x) => {
    ...
    if (t.patchOnly) {
      ...
      if (t.elevation > support + 1) why.push(`I1 fillet at ${t.elevation} on support ${support}`);
...
describe('the maps that were reported broken', () => {
  it('each fixture still holds the fault it was reported for', () => {
    ...
    expect(found).toEqual([
      ...
      'report-3.json: 3 bad fillets, 1 pits',
...
describe('a pit left in a map', () => {
  it('takes the paint that fills it, one tier at a time', () => {
    // report-4's fault: cells the brush could not fill, because the planner read a Γ fillet's
    // cosmetic tier as if it were support and started the ladder above the ground. The rung that
    // was skipped was refused for having no base, the cell stayed out of the mass, and the fillet
    // it could not replace was dropped later — leaving the hole.
