 ...
 *
 * Erasing elevated water used to dig the cell to bare ground, which unbanks the water beside it —
 * the containment rule then refuses the stroke whole, so the edge of a waterfall could never be
 * taken back. The water eraser now paints MOUNTAIN at the water's own elevation: the cell becomes
 * this layer's terrain and is itself the bank that keeps the neighbours legal. The mountain brush
 * gets the same stance: water is changed to terrain in place, never treated as a floor to stack on.
 */
