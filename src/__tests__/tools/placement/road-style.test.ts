...
import { readRoadStyle, bendDensity, turnPenaltyFor, STYLE_MIN_ROADS } from '../../../tools/placement/road-style';
import { astar } from '../../../tools/placement/network';
 ...
 *  reads `state.objects`, never validates. */
function layRoad(state: GridState, id: string, x: number, y: number, catalogId = 'path-simple-brick'): void {
...
describe('readRoadStyle', () => {
  it('a map is measured off its own roads: a grid reads low, a ramble reads high', () => {
  ...
  it('a grid\'s measured style buys the full turn penalty, a ramble\'s buys none', () => {
    const grid = makeState(W, H);
    layGrid(grid);
    const ramble = makeState(W, H);
    layRamble(ramble);
    expect(readRoadStyle(grid).turnPenalty).toBe(turnPenaltyFor(readRoadStyle(grid).naturalness));
    expect(readRoadStyle(grid).turnPenalty).toBeGreaterThan(readRoadStyle(ramble).turnPenalty);
  });
  ...
  it('the material is the nearest street\'s, not the pool\'s first', () => {
    const state = makeState(W, H);
    layRoad(state, 'slate', 5, 5, 'path-park-stone');
    layRoad(state, 'brick', 30, 30, 'path-simple-brick');
    expect(readRoadStyle(state, { x: 6, y: 6 }).materialId).toBe('path-park-stone');
    expect(readRoadStyle(state, { x: 29, y: 29 }).materialId).toBe('path-simple-brick');
  });
...
describe('a rectilinear map yields a rectilinear route (readRoadStyle feeding astar)', () => {
  it('the rectilinear turnPenalty (naturalness 0) turns strictly less than the organic default (naturalness 1) over the same obstacle field', () => {
    const W = 60, H = 60;
    ...
    const bendsAt = (naturalness: number): number => {
      const tp = turnPenaltyFor(naturalness); // readRoadStyle's own mapping
