 ...
 * anchors — not a patch over the old one. A mountain curve stacks on what is under it, so painting
 * over the old path would raise it twice, and a cell the old path covered has to go back to what it
 * held before the curve rather than to bare ground.
 */
...
describe('moving an anchor', () => {
  ...
  it('does not stack the mountain twice where the paths overlap', () => {
    // Restore-then-relay, not patch: painting over the old path would raise those cells again.
    const { state, ctx, tool } = world();
    draw(tool, ctx, [{ x: 4, y: 10 }, { x: 12, y: 10 }, { x: 20, y: 10 }]);
...
describe('a tweak the rules refuse', () => {
  ...
  it('reverts a tweak that could lay nothing at all', () => {
    // The other failure: every command refused pre-command, so the stroke removed the old curve and
    // laid no new one. No post-stroke violation is reported for that, only an empty result.
    const { state, ctx, tool } = world();
...
describe('tile curves', () => {
  ...
  const roads = (state: ReturnType<typeof makeState>) => [...state.objects.values()]
    .filter((o) => o.catalogId.startsWith('road'));
...
describe('a path that clips ground it may not build on', () => {
  ...
  it('lays the part it can, instead of nothing at all', () => {
    // The plan batches a shape into ONE command per elevation, and a command is refused WHOLE if
    // any single cell in it is — so a path that so much as clipped the sea used to paint nothing.
  ...
  it('clears the OLD curve when adjusting, even where it crossed the sea', () => {
    // The baseline covers every cell the curve ASKED for, forbidden ones included, and one of those
    // in a batch refused the whole restore — so the old curve stayed while the new one was laid
    // over it, and the map ended up with both.
    const { state, ctx, tool } = zonedWorld();
    draw(tool, ctx, [{ x: 10, y: 20 }, { x: 30, y: 20 }]);
    const laid = painted(state);
