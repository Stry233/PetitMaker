 ...
 * paint brushes (drawing-tool.ts). Omitting the argument defaults it to `true` (micro), which sits a
 * tile-mode erase ghost half a tile off from the cell the click would erase.
 */
...
describe('EraserTool: ghost grid alignment', () => {
  ...
  it('the erase HIT TARGET (not just the ghost) is macro-cell-accurate in tile mode: same coords in, same cell erased', () => {
    // eraseTileCells/getCell both index the cells array by the plain macro (x,y) the pointer machine
    // resolves, so there is no separate micro-space hit test to get wrong — unlike the ghost's PIXEL
    // offset above.
    const state = makeState(10, 10);
