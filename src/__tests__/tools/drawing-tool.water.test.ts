...
describe('DrawingTool: with no layer pinned, water follows the ground under it', () => {
  ...
  // DELIBERATE REVERSAL of the old "the origin cell decides the whole stroke" pin: a footprint that
  // straddles a step used to plan every cell at the clicked layer, so the far side was refused for
  // standing at a height nobody asked about.
  ...
  // THE POINT OF THE CHANGE, end to end: the same dab used to lay NOTHING. Every cell was planned
  // at the clicked layer, so the eight ground cells failed V-WTR-01 for having nothing to stand on
  // and took the whole command down with them.
...
describe('DrawingTool: with a layer PINNED, water paints at that layer wherever the ground reaches it', () => {
  ...
  // DELIBERATE FLIP of "skips where the pinned layer is illegal": ground the pin cannot reach is
  // where a stroke used to stop, and a lake pinned at 5 broke into puddles on the terraces that
  // happened to be high enough. It follows the ground there now.
