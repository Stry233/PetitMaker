...
describe('a road records the structural surface, not a fillet corners cosmetic tier', () => {
  it('planTile (fresh placement, and the ghosts own preview via planPaint) reads the real surface', () => {
    ...
    // The plan a click issues IS the ghost's own preview (planPaint), so this one fix covers both.
  ...
  it('reconcileRoadsAfterMountainPaint re-elevates an already-placed road to the real surface', () => {
    const state = makeState(10, 10);
    const road: PlacedObject = { id: 'road-4-4', catalogId: 'road-dirt', position: { x: 4, y: 4 }, rotation: 0, elevation: 0 };
    ...
    reconcileRoadsAfterMountainPaint(touched.map(([x, y]) => ({ x, y })), ctx);

    const reconciled = [...state.objects.values()].find((o: any) => o.catalogId === 'road-dirt') as PlacedObject | undefined;
