...
function world(autoEdgeCut: AutoEdgeCut, mode: 'line' | 'rect' | 'brush' = 'line') {
  ...
  const ghosts: Ghost[] = [];
  const ctx = {
    ...makeToolCtx(state, executor, 1, 1, { autoEdgeCut, contentType: 'tile', tileMaterial: 'path-overgrown-dirt' }),
    overlay: {
...
function pave(w: ReturnType<typeof world>, cells: MacroCoord[]): void {
  for (const c of cells) {
    w.executor.execute({
      ...
      object: { id: `built-${c.x}-${c.y}`, catalogId: 'path-overgrown-dirt', position: c, rotation: 0, elevation: 0 },
      loadValue: 0,
    });
  }
}
...
describe('the road ghost sees its own stroke', () => {
  ...
  it('a dab reconnecting to a tile an earlier dab of the same stroke cut', () => {
    ...
    // The third dab is the one a stale cache answers empty for: (21,22) still carries the end-cap it was
    // given while it WAS the end.
