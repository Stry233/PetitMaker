...
async function select(id: string) {
  ...
  useEditorStore.setState({ selectedItemId: id });
}

describe('ObjectPlacerTool: bridge + ramp placement (double-snap regression)', () => {
  it('places a bridge across a painted water river', async () => {
    await select('bridge-teak'); // width 2, waterSpan 3-6
    try {
      ...
      const ex = exec(state);
      new ObjectPlacerTool().onPointerDown(m(11, 11), m(11, 11), makeToolCtx(state, ex));
      expect(objectsByCatalog(state, 'bridge-teak').length, 'bridge placed across the river').toBe(1);
    } finally {
      const { useEditorStore } = await import('../../../state/store');
      useEditorStore.setState({ selectedItemId: null });
    }
  });

  it('places a ramp up a cliff in every drop direction', async () => {
    ...
    for (const s of setups) {
      await select('ramp-teak-stair'); // width 2, height 4, heightDrop 1
      try {
        ...
        const ex = exec(state);
        new ObjectPlacerTool().onPointerDown(s.at, s.at, makeToolCtx(state, ex));
  ...
  it('rejects a ramp on a too-short (3-block) lower terrace — incl. the ghost; accepts a 5-block one', async () => {
    await select('ramp-teak-stair'); // width 2, spanLen 4
    try {
      ...
      new ObjectPlacerTool().onPointerDown(m(10, 10), m(10, 10), makeToolCtx(tooShort, exS));
      ...
      const ok = build(5);
      const exOk = exec(ok);
      new ObjectPlacerTool().onPointerDown(m(10, 10), m(10, 10), makeToolCtx(ok, exOk));
