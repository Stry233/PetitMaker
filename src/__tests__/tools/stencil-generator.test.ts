...
import { COVERAGE_ON, covered, FLAT_SAFE_ELEVATION, insetDepth, nearestTerrain, relaxHeights, smoothShape, terrainPalette, type Stencil } from '../../tools/generation/stencil';
import { layStencilColor, layStencilObjects, layStencilTerrain, tilesAShape } from '../../tools/generation/stencil-generator';
import { stencilChooser } from '../../tools/generation/stencil-trim';
...
describe('stencil as SHAPE (the text mode)', () => {
  it('raises the covered cells and leaves the rest alone', () => {
    ...
    const res = layStencilTerrain(state, { origin: { x: 5, y: 5 }, stencil }, TerrainType.Mountain, 3, (c) => exec(state).execute(c));
  ...
  it('builds bottom-up, so a tall shape stands on its own mass', () => {
    const state = makeState(20, 20);
    const rows = Array.from({ length: 6 }, () => '######');
    const e = exec(state);
    layStencilTerrain(state, { origin: { x: 4, y: 4 }, stencil: stencilOf(rows) }, TerrainType.Mountain, 4, (c) => e.execute(c));
    // The middle of a 6x6 slab can carry the full height: it has its 3x3 base.
    expect(at(state, 7, 7)?.elevation).toBe(4);
    // And it is legal by the live rules, which is the point of climbing a layer at a time.
...
describe('the trim chooses each corner from the source', () => {
  ...
  it('chamfers a staircase, whatever the quadrants say', () => {
    // On a quantised diagonal the bevels chain into one straight line; fans scallop and a mix
    // wobbles. The edge direction comes from the coverage gradient, so the quad values — mid-range
    // and noisy exactly there — no longer pick the shape.
    const s = stencilOf([
      '#....',
      '##...',
      '###..',
...
describe('the region, not its bounding box', () => {
  ...
  it('writes nothing outside the cells it was allowed', () => {
    ...
    layStencilTerrain(
      ...
      TerrainType.Mountain, 3, (c) => e.execute(c),
    );
    for (let y = 4; y < 10; y++) {
      for (let x = 4; x < 10; x++) {
...
describe('what the rules refuse, refused before it is asked', () => {
   ...
   */
  it('measures inset in the 8-neighbourhood, because a 3x3 base is what V-MTN-03 asks for', () => {
    // A plus: its centre is two steps in by 4-neighbour counting, but its corners are OUTSIDE the
    // shape, so it has no complete 3x3 and must not be told it may climb.
    const s = stencilOf([
      '.#.',
      '###',
      '.#.',
    ]);
    const depth = insetDepth(s);
    expect(depth[1 * 3 + 1]).toBe(1); // the centre is still on the outline, diagonally
  });

  it('a solid block does have a complete 3x3, so its middle may climb', () => {
    const depth = insetDepth(stencilOf(['###', '###', '###']));
    expect(depth[1 * 3 + 1]).toBe(2);
  });
  ...
  it('a letter with thin strokes commits clean against the live rules', () => {
    ...
    const rows = ['..##..', '..##..', '..##..', '..##..', '..##..', '..##..'];
    layStencilTerrain(state, { origin: { x: 6, y: 6 }, stencil: stencilOf(rows) }, TerrainType.Mountain, 8, (c) => e.execute(c));
    expect(e.commitStroke(0)).toEqual([]);
    expect(at(state, 8, 8)).not.toBeNull();   // and it is still standing
  });

  it('climbs no higher than a zone hole inside the shape can support', () => {
    // A beach cell inside the glyph is never laid, so the cells around it have no complete 3x3
    // there. V-MTN-03 is a POST-stroke rule: nothing refuses during the lay, and a target height
    // that ignores the hole is found out only at commit, as an auto-revert of the whole run.
    const state = makeState(24, 24);
  ...
  it('climbs no higher than an allow-clipped cell can support', () => {
    const state = makeState(24, 24);
    const e = exec(state);
    const rows = Array.from({ length: 8 }, () => '########');
    const allow = new Set<number>();
    ...
    layStencilTerrain(state, { origin: { x: 6, y: 6 }, stencil: stencilOf(rows), allow }, TerrainType.Mountain, 8, (c) => e.execute(c));
    expect(e.commitStroke(0)).toEqual([]);
  });

  it('comes down around a cell an object refuses mid-climb', () => {
...
describe('stencil as COLOUR (the image mode)', () => {
  ...
  it('lays a blue picture as water at ground level, and commits legally', () => {
    ...
    const e = exec(state);
    const rows = Array.from({ length: 4 }, () => '####');
    layStencilColor(state, { origin: { x: 6, y: 6 }, stencil: stencilOf(rows, hexStringToNumber(WATER_COLOR)) }, 8, (c) => e.execute(c));
