...
- **Your AI Agent session.** *What it contains:* the orders, assistant responses, tool results, plans, approval answers, filed records, and the IDs of cleared records used to restore the assistant panel after a reload. Clearing a record removes its transcript and affected conversation summaries from browser storage and future assistant requests. Provider-native response blocks and tool-result images are excluded, and reasoning is reduced to a short excerpt. *When it is written:* as the session changes. *How to remove it:* clear individual records or past jobs in the assistant panel, start a fresh AI Agent session, clear your browser's site data, or use Settings then Local data.
...
Before added text appears in an image preview or export, the application checks its title, description and footer on your device. JSON exports use the same check for title, description and author when Notes is enabled. The check uses third-party word-matching packages and derived political-topic matching data bundled with the application, without downloading AI models. Website links in added text are also checked locally; the application does not visit those links. The text is not uploaded for this check, and the reviewer does not persist a separate copy of the text or its result. Package matchers and temporary review data may remain in memory for up to thirty idle seconds to make subsequent edits faster; closing the panel clears them. Editing the text, closing the panel or cancelling an export stops the current check. You can also export without added text.

Before displaying an image export preview, and when you request a JSON export, the application also checks a local rendering of the map. It downloads image-classification and text-recognition models from the same site as the application and runs them on your device. Map images and recognized text are not uploaded or separately saved by this check. Model files may remain in the browser's HTTP cache; temporary analysis data is released after checking or cancellation. The check can miss content or make mistakes, and an incomplete check is identified in the export panel.
...
During connection setup and model management, the AI Agent also downloads public model capability information from **Models.dev**. This request sends no API key, prompt, map data, or cookies. Models.dev receives ordinary network request metadata such as your IP address. The application retains a recent copy in memory so a catalog outage does not prevent use.
