...
export class ToolManager {
   ...
   *  `pointerKnown`, so no `handlePointerMove` precedes it and the mirror would otherwise answer
   *  from whenever the pointer last moved). `refreshCtx` is a field copy, so refreshing per ask is
   *  cheap. Read-only by convention: nothing here mutates the arming itself. */
