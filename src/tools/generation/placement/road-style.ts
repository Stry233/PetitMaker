 ...
 * street through an organic valley. Three facts are readable without asking anyone:
 *
 *  - the RECIPE, when the map carries one (`GridState.generation`, set by a full Generate and by a
 *    procedural-v1 import) — `naturalness` is the same dial `geoStyle` already turns for the
 *    generator's own router, so a generated map's new road is routed by its own recipe;
 *  - the ROADS THEMSELVES otherwise, by measured bend density: what fraction of paved cells turn.
 *    A hand-drawn grid measures near zero, a hand-drawn ramble near one;
 ...
 *
 * The macro used to take none of them: `roads.ts` calls `makeCtx(state, …)` without a naturalness,
 * so every macro-laid road was routed at the organic default whatever the map underneath it was.
 */
...
export interface RoadStyle {
  /** A* cost per direction change, in `astar`'s own units (`geoStyle(naturalness).turnPenalty`). */
  turnPenalty: number;
  ...
  source: 'recipe' | 'measured' | 'default';
}

/** A fully organic generated network measures around here (bendDensity), so `measured / BEND_ORGANIC`
 *  reads a hand-built network on the same 0..1 scale `geoStyle` already turns. Named and unit-tested
...
/** Reads the map's own road character: the recipe if it carries one, else what is measured off its
 *  own paved cells, else the organic default a bare map has always routed at. Walks `state.objects`
 *  once through `state/catalog` for the road category (the same read `roads.ts` makes). */
export function readRoadStyle(state: GridState, near?: MacroCoord): RoadStyle {
  ...
  if (state.generation?.naturalness !== undefined) {
    naturalness = state.generation.naturalness;
    source = 'recipe';
  } else if (roadCells.size >= STYLE_MIN_ROADS) {
  ...
  return {
    turnPenalty: geoStyle(naturalness).turnPenalty,
    naturalness,
