 ...
 * pointed at. They shared one word and are not the same thing: the old model could not express the
 * second at all, so aiming at the plaza silently moved the exit to the map's edge.
 *
 ...
 *
 * THE TERRAIN AROUND A MAZE IS NOT TOUCHED. An earlier draft had the generator wall a maze whose
 * ends were both outside; a probe disproved it (`__tests__/tools/maze-endpoints.test.ts`, committed
 * failing). Two holes in the boundary of a simply-connected shape are two doors onto the SAME
 * outside, so no wall the generator may build separates them. The inward pairing this file defaults
 * to has no such problem and needs nothing: with an end inside, entering is the only way to arrive,
 ...
 * flat-array passes over a maze's own rectangle, which is milliseconds, and the approximation
 * measured one cell short of the best pair on the first map it was tried on.
 */
