...
import { loadLexicon, takeUnconfirmed } from '../text/segmenter';
import { crop } from './raster';

/** A provisional phrase hit loads the segmentation lexicon once, then the reading is evaluated again. */
async function confirmed(evaluate: () => boolean): Promise<boolean> {
  const verdict = evaluate();
  if (!takeUnconfirmed()) return verdict;
  await loadLexicon();
  return evaluate();
}
  ...
  try {
    ...
    for (const canvas of views) {
      ...
      for (const line of lines) {
        if (!(await confirmed(() => restrictedReading(line.text, line.confidence)))) continue;
        ...
        await worker.setParameters({ tessedit_pageseg_mode: PSM.SPARSE_TEXT });
        if (await confirmed(() => confirmedReading(line.text, check.data.text, check.data.confidence))) return true;
      }
    }
    return false;
