...
// The store is four independently-typed slices composed at one `create()` call. Each factory
// types its own `set`/`get` no wider than the fields it actually touches (see the cross-slice
// `Deps` picks in `edit.ts`/`engine.ts`), so a slice naming a field outside its declared type is
// a compile error rather than a convention. `shell` declares no engine field at all, which is
// the boundary a shell replaces. The check is the type system's, so a cast still defeats it.
export type EditorStore = PrefsSlice & ShellSlice & EditSlice & EngineSlice;
