 ...
 * on both canvases and on every DOM surface: the code-drawn SVG set in `cursor-art-classic.ts` for
 * the pixel-art PNG set in `cursor-art.ts`. Both are complete over the `CursorId` catalogue and each
 * carries its own hotspots, since a hotspot belongs to a drawing rather than to an id.
 *
 ...
 */
export const USE_CLASSIC_CURSORS = true;
