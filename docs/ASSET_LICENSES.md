...
Political-topic matching uses derived data in `src/assets/moderation/political-index.json` from @zhin.js/sensitive-filter 2.0.1 (MIT), fwwdn/sensitive-stop-words (Apache-2.0), and houbb/sensitive-word-data (Apache-2.0). Selected categories are normalized, filtered, deduplicated and replaced with fingerprints. Tone-free character readings and pronunciation fingerprints derive from pinyin-pro 3.29.3 (MIT). These transformations are documented in `src/assets/moderation/provenance.json`; they are lexical matching data, not learned model weights or encrypted secrets. Source links, copyright notices and full licenses appear in `THIRD_PARTY_NOTICES.md` and `licenses/`.
