...
[![立即体验 petit-maker.com](https://img.shields.io/badge/%E2%96%B6%20%E7%AB%8B%E5%8D%B3%E4%BD%93%E9%AA%8C-petit--maker.com-FFB347?style=flat-square&labelColor=43413F)](https://petit-maker.com)
...
<img src="./media/hero-annotated.zh.png" alt="谷地工坊编辑器中的一座小岛，菜单已展开，图上有四处标注：左侧展开的卡片标为「菜单」，右上角标为「图层面板」，右下角标为「缩放」，左下角标为「撤销与重做」" width="840">

<sub>上面这座岛，可以在下文直接下载带走。</sub>
...
- **正式站点：**<https://petit-maker.com>
- **代码仓库：**<https://github.com/Stry233/PetitMaker>

## 两个视图，同一张地图

<div align="center">
<img src="./media/two-views.zh.gif" alt="同一片地图区域在 2D 视图与 3D 视图之间淡入淡出切换的动图：抬高一级的台地上新种了三棵树，两个视图里选中的都是同一棵" width="440">

<sub>同一处改动，两个视图：抬高一级的台地、上面新种的三棵树，切换之后选中的还是同一棵。</sub>
</div>
...
其余的工具用来调细节：边角切割每次修一个山角，也可以打开自动修边，边画边修，形状在切角和圆角里选一种；橡皮每次削掉一层；物品吸附网格，点一下旋转，按住拖走；桥需要两岸平整且高度相同，条件满足时会自动对齐。图层各有显示和锁定开关，自动保存会保留上次的进度，界面提供七种语言。不需要账号，也没有服务器。
...
<div align="center">
<img src="./media/island-growth.zh.gif" alt="生成过程的动画分镜，每一步在左下角标出步骤名，并以从左到右的擦除切换到下一步：空白地图、粉彩色的街区规划、层叠台地、湖泊与河流、逐级跌落的瀑布、桥梁道路与房屋，最后是树木与花草" width="300">

<sub>一个配方号，七个阶段：先分区，再垒台地，然后引河道下山，最后铺路建房。</sub>
</div>

两种生成器都只要一个配方号，走的都是和你的画笔同一套规则。同一个号码永远生成同一张图，所以喜欢哪个号码，直接发给朋友就行。

<div align="center">
<img src="./media/algorithms.zh.png" alt="两张整图并排，分别标注「随机」和「迷宫」：左边是一座建好的岛，有层叠台地、道路和数百件物品；右边是同一张地图铺满了一格宽的山体迷宫，中央是广场" width="620">

<sub>同一个配方号，两种生成器：<b>随机</b>造一座有人住的岛，<b>迷宫</b>把同一片地面铺成通道。</sub>
</div>

**随机**会设计一座岛：主题各异的街区、层层能走上去的台地、经过几级瀑布流入海里的河道、两岸等高处的桥，以及连到每一户的道路。想改它的形态，动一根滑杆就够了：自然度。

<div align="center">
<img src="./media/naturalness-strip.png" alt="同一个配方的三种生成结果：自然度 0 时湖泊方正、街道笔直，50 时半规整，100 时河湾与海湾都是自然的曲线" width="620">

<sub>从方正到自然：<b>自然度</b> 0 · 50 · 100，同一个配方。左边湖岸方正、街道笔直，右边轮廓和道路都带自然的转折。</sub>
</div>

**迷宫**用递归回溯算法在可建造的地面上开出一座迷宫，任意两条通道都相通。它的墙体就是普通山体，生成之后照样能画、能切角、能装饰；它也认选区，所以迷宫可以只占地图的一角。
...
<div align="center">
<img src="./media/providers.zh.png" alt="一条由九块牌子组成的色带，用编辑器自己的米色与墨色排版，每块牌子上是该平台的品牌标识，衬在它自己颜色的圆角徽章里，旁边是平台名称：Anthropic、OpenAI、DeepSeek、Google、OpenRouter、智谱、阿里云、月之暗面，以及最后一块更宽的「自定义」，配一枚齿轮徽章，写着任何 OpenAI 兼容接口都行（Ollama、LiteLLM、校园网关）" width="900">

<sub>九个平台任选；最后一行是任何兼容 OpenAI 接口的服务，包括跑在你自己机器上的模型。</sub>
</div>
...
<div align="center">
<img src="./media/sitelog.zh.png" alt="智能体面板的三张截图并排：第一张是拟好的蓝图，三个阶段都还没勾选，下方是「就这样，开工」和「暂时不用」；第二张是「可以吗？」卡片，带允许、始终允许、跳过三个按钮，下方阶段列表里第一阶段已勾选，第二阶段带一行回滚说明；第三张是完工状态，各阶段全部勾选，一张「完成，共 6 步」的卡片提供「全部撤销」，最后是智能体的留言" width="900">

<sub>同一轮施工的三个瞬间：方案等你点头、回滚之后的一步在等许可、完工时每个阶段仍是一个回退点。卡片是编辑器对真实工具调用的记录；模型说的话是为这张图写的。</sub>
</div>
...
<picture>
  ...
  <img src="./media/underhood-zh-light.svg" alt="源码分层框图：一层一条通栏色带，每条里排满该层的模块。最上面是「谁在编辑」（画笔、生成器、AI 智能体），往下依次是 agent、io、api，ui，canvas（2D 编辑器与 3D 编辑器），i18n 与 legal，tools，rules，state，core，最底下是 config 与 assets。一条高亮通道从三个编辑者向下穿过 rules 落到 core/commands；右侧竖排标注：依赖只向下" width="900">
</picture>
</div>
